...
const CustomPackages = () => {
  ...
  const fetchPackages = async () => {
    ...
    setLoading(true);
    try {
      const response = await apiCall(`/custom-packages?page=${page}&limit=${limit}&search=${encodeURIComponent(searchTerm)}`);
  ...
  const confirmDelete = async () => {
    ...
    try {
      const response = await apiCall(`/custom-packages/${deleteTargetId}`, 'DELETE');
      const data = await response.json();
      if (data.success) {
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      const url = selectedPackage ? `/custom-packages/${selectedPackage.id}` : '/custom-packages';
  ...
  const columns = [
    {
      ...
      render: (row) => (
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-indigo-500 to-cyan-500 flex items-center justify-center text-white shadow-sm shrink-0">
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">Manage custom pricing tiers for specific clients.</p>
        </div>
